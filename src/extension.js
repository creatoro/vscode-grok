...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.askWorkspace', async () => {
        ...
        if (showPreview === 'always' || showPreview === 'workspace-only') {
            const proceed = await vscode.window.showInformationMessage(`Sending workspace data (${chunks.length} chunks, total ~${workspaceContent.length} chars). Proceed?`, 'Yes', 'No');
            if (proceed !== 'Yes')
                return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.askCurrentTab', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.askFunctionUnderCursor', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return vscode.window.showWarningMessage('No active editor');
        ...
        if (!range)
            return vscode.window.showWarningMessage('No function under cursor');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.askSelectedText', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return vscode.window.showWarningMessage('No active editor');
        const content = editor.document.getText(editor.selection);
        if (!content)
            return vscode.window.showWarningMessage('No text selected');
        const apiKey = vscode.workspace.getConfiguration('vscodeGrok').get('apiKey', '');
        const model = vscode.workspace.getConfiguration('vscodeGrok').get('model', 'grok-4');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.searchWorkspace', async () => {
        const apiKey = vscode.workspace.getConfiguration('vscodeGrok').get('apiKey', '');
        const model = vscode.workspace.getConfiguration('vscodeGrok').get('model', 'grok-4');
    ...
    const completionProvider = vscode.languages.registerInlineCompletionItemProvider({ scheme: 'file', language: '*' }, {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vscodeGrok.openChatPanel', () => {
        const panel = vscode.window.createWebviewPanel('grokChat', 'Grok Chat', vscode.ViewColumn.Beside, { enableScripts: true });
...
async function callXAIAPI(prompt, model, apiKey) {
    const response = await (0, node_fetch_1.default)('https://api.x.ai/v1/chat/completions', {
        method: 'POST',
        headers: {
